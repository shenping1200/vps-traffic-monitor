...
def record_sample() -> dict:
    ...
    with sqlite3.connect(DB_PATH) as connection:
        ...
        for iface, counters in interfaces.items():
            ...
            if previous:
                ...
                connection.execute("""
                    INSERT INTO daily_usage (day, iface, rx, tx)
                    VALUES (?, ?, ?, ?)
                    ON CONFLICT(day, iface) DO UPDATE SET
                        rx = rx + excluded.rx,
                        tx = tx + excluded.tx
                """, (current_day, iface, rx_delta, tx_delta))
            connection.execute("""
    ...
    day_usage = usage_for_day(current_day)
    month_usage = usage_for_month(current_month)
    try:
        evaluate_alerts(day_usage, month_usage)
    except Exception:
        pass
    return {
        "timestamp": int(now),
...
def test_telegram(vps_traffic_session: Optional[str] = Cookie(default=None)) -> dict:
    require_auth(vps_traffic_session)
    settings = get_settings()
    if not settings.get("tgToken") or not settings.get("tgChatId"):
        raise HTTPException(status_code=400, detail="请先在设置中心填写 Telegram Token 和 Chat ID")
    message = (
        f"✅ <b>VPS 流量监控测试消息</b>\n\n"
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Telegram 发送失败：{error}") from error
